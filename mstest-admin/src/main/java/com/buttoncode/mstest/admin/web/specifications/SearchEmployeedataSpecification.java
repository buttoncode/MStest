...
public class SearchEmployeedataSpecification implements Specification<Employeedata> {

    private EmployeeSearchForm employeeSearchForm;
    private List<Predicate> predicates;

    public SearchEmployeedataSpecification(EmployeeSearchForm employeeSearchForm) {
        this.employeeSearchForm = employeeSearchForm;
        predicates = new ArrayList<Predicate>();
    }

    @Override
    public Predicate toPredicate(Root<Employeedata> root, CriteriaQuery<?> criteriaQuery, CriteriaBuilder criteriaBuilder) {

        if (!Strings.isNullOrEmpty(String.valueOf(employeeSearchForm.getNumbersap()))) {
            predicates.add(criteriaBuilder.equal(root.get("employee").get("numbersap"), employeeSearchForm.getNumbersap()));
        }
        if (!Strings.isNullOrEmpty(String.valueOf(employeeSearchForm.getFirstname()))) {
            predicates.add(criteriaBuilder.equal(root.get("employee").get("firstname"), employeeSearchForm.getFirstname()));
        }
        if (!Strings.isNullOrEmpty(String.valueOf(employeeSearchForm.getLastname()))) {
            predicates.add(criteriaBuilder.equal(root.get("employee").get("lastname"), employeeSearchForm.getLastname()));
        }
        ...
        if (!Strings.isNullOrEmpty(String.valueOf(employeeSearchForm.getStatusemployee()))) {
            predicates.add(criteriaBuilder.equal(root.get("employee").get("statusEmployee").get("name"), employeeSearchForm.getStatusemployee()));
        }
